...
st.set_page_config(page_title="BBQ 발주 분석 V5", layout="wide")
...
def get_engine():
    ...
    try:
        eng = create_engine("sqlite:///bbq_order_history_v5.db", future=True, pool_pre_ping=True)
        with eng.connect() as conn:
            conn.execute(text("SELECT 1"))
...
def ensure_column(conn, table_name: str, column_name: str, ddl_sql: str):
...
def init_db():
    ...
    try:
        with eng.begin() as conn:
            ...
            ensure_column(conn, "order_lines", "uploader_name", "ALTER TABLE order_lines ADD COLUMN uploader_name TEXT")
            ensure_column(conn, "order_lines", "upload_key", "ALTER TABLE order_lines ADD COLUMN upload_key TEXT")

            conn.execute(text("""
                UPDATE order_lines
                SET uploader_name = '기존데이터'
...
def make_upload_key(source_date: str, uploader_name: str) -> str:
    clean_name = uploader_name.strip() if uploader_name else "미지정"
    return f"{source_date}_{clean_name}"
...
def latest_uploads():
    x = q("""
        ...
        FROM order_lines
        ORDER BY source_date DESC, uploader_name
    """)
    if x.empty:
        return []
...
def latest_upload_keys_by_user(uploader_name, current_date=None, limit=3):
    if current_date:
        df = q("""
            SELECT DISTINCT upload_key, source_date
            FROM order_lines
            WHERE uploader_name = :u
              AND source_date < :d
            ORDER BY source_date DESC
        """, {"u": uploader_name, "d": current_date})
    else:
        df = q("""
            SELECT DISTINCT upload_key, source_date
            FROM order_lines
    ...
    return df["upload_key"].tolist()[:limit]


def load_detail_by_upload_key(upload_key):
    return q("""
        SELECT
            store_code AS 매장코드,
            store_name AS 매장명,
...
def load_detail_by_date_all(source_date):
    return q("""
        SELECT
            store_code AS 매장코드,
            store_name AS 매장명,
...
def save_order_history(source_date, uploader_name, source_name, detail_df):
    eng, _, _ = get_db_status()
    if eng is None:
        return False, "DB 엔진 없음"
...
def assign_store(store_code, store_name, bm_name):
    ok = exec_sql("DELETE FROM store_assignments WHERE store_code = :c", {"c": int(store_code)})
    eng, _, _ = get_db_status()
    if eng is None:
        return False
...
st.title("BBQ 발주 분석 V5")
st.caption("업로더 기준 저장 / 운영2팀 총합 / Supabase 연결 안정화 버전")

if db_ok:
    if db_mode == "external":
        st.success("외부 DB 연결 정상")
    elif db_mode == "sqlite":
        st.warning("외부 DB 연결 실패로 로컬 sqlite 모드로 실행 중입니다.")
else:
    st.error(f"DB 초기화 실패: {db_msg}")

if db_err:
    st.caption(f"DB 참고: {db_err}")
...
if menu == "오늘 발주 점검":
    ...
    if not dates:
        ...
        st.dataframe(
            urgent_df[["BM", "매장명", "코스", "미발주항목", "감소항목", "오일판정", "소비기한리스크", "총평"]].head(50),
            use_container_width=True,
            height=500
        )
