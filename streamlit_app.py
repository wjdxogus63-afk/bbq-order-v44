...
def make_upload_key(source_date, uploader_name):
    return f"{source_date}_{uploader_name}"
...
def latest_dates():
    ...
    if x.empty or "source_date" not in x.columns:
        return []
...
def save_order_history(source_date, uploader_name, source_name, detail_df):
    ...
    rows = rows[[
        ...
        "store_code", "store_name", "course", "product_code", "product_name", "qty", "conv", "converted_qty"
    ]]
...
st.caption("업로더별 저장 반영 / 같은 날짜도 날짜+이름 기준으로 별도 저장")
...
if menu == "오늘 발주 점검":
    ...
    if uploaded is not None and user_name:
        ...
        upload_key = make_upload_key(source_date, user_name)
        st.info(f"저장키: {upload_key}")
        ...
        if st.button("분석 실행", type="primary", use_container_width=True):
            ...
            if save_flag:
                save_order_history(source_date, user_name, uploaded.name, curr_detail)
                st.success(f"공용 DB 저장 완료: {upload_key}")

            excel_bytes = to_excel_bytes(summary_df, curr_detail, prev1_detail, prev2_detail, prev3_detail)
            st.download_button(
                "엑셀 다운로드",
                data=excel_bytes,
                file_name=f"발주점검_{upload_key}.xlsx",
    ...
    if not uploads:
        ...
        uploader_uploads = [x for x in uploads if x["uploader_name"] == user_name] if user_name else uploads
        if not uploader_uploads:
            st.info("현재 사용자 기준 저장된 발주가 없습니다.")
        else:
            options = {f"{x['source_date']} / {x['uploader_name']}": x["upload_key"] for x in uploader_uploads}
            selected_label = st.selectbox("업로드 선택", list(options.keys()))
            selected_key = options[selected_label]
            curr_meta = next(x for x in uploader_uploads if x["upload_key"] == selected_key)
            curr_detail = load_detail_by_upload_key(selected_key)
            prev_keys = latest_upload_keys_by_user(curr_meta["uploader_name"], curr_meta["source_date"], limit=3)
            prev1_detail = load_detail_by_upload_key(prev_keys[0]) if len(prev_keys) > 0 else None
            prev2_detail = load_detail_by_upload_key(prev_keys[1]) if len(prev_keys) > 1 else None
            prev3_detail = load_detail_by_upload_key(prev_keys[2]) if len(prev_keys) > 2 else None
            summary_df = build_analysis_summary(curr_detail, prev1_detail, prev2_detail, prev3_detail)
    ...
    if not dates:
        ...
        if selected_store:
            hist = q("""
                ...
                FROM order_lines
                WHERE store_name = :s
                GROUP BY source_date, uploader_name
                ORDER BY source_date, uploader_name
            """, {"s": selected_store})
            st.dataframe(hist, use_container_width=True, height=550)
            st.info(ai_store_direction(hist.rename(columns={"업로더": "담당자"})))
    ...
    if not dates:
        ...
        st.dataframe(raw_uploads, use_container_width=True, height=250)
    ...
    if not user_name:
        ...
        latest_detail = load_detail_by_date_all(dates[0])
    ...
    if uploads:
        ...
        selected_label = st.selectbox("삭제할 업로드 선택", list(options.keys()))
        selected_key = options[selected_label]
        if st.button("선택 업로드 삭제"):
            exec_sql("DELETE FROM order_lines WHERE upload_key = :k", {"k": selected_key})
            st.success(f"{selected_key} 삭제 완료")
            st.rerun()
