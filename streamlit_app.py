...
st.set_page_config(page_title="BBQ 발주 분석 V4", layout="wide")
...
def get_engine():
    try:
        ...
        db_url = None
    db_url = db_url or os.getenv("DATABASE_URL") or "sqlite:///bbq_order_history_v4.db"
...
def normalize_order_df(file):
    ...
    grouped = work.groupby(
        ["매장코드", "매장명", "코스", "제품코드", "제품명"], as_index=False
    )["합계"].sum().copy()
...
def latest_dates():
    ...
    return x["source_date"].tolist()


def load_detail_by_date(source_date):
    return q("""
        SELECT
            store_code AS 매장코드,
            store_name AS 매장명,
            ...
            qty AS 합계,
            conv AS 환산기준,
            converted_qty AS 환산수
        FROM order_lines
        WHERE source_date = :d
    """, {"d": source_date})


def save_order_history(source_date, source_name, detail_df):
    exec_sql("DELETE FROM order_lines WHERE source_date = :d", {"d": source_date})
    ...
    rows = rows[[
        "source_date", "source_name", "store_code", "store_name", "course",
        "product_code", "product_name", "qty", "conv", "converted_qty"
    ]]
    rows.to_sql("order_lines", get_engine(), if_exists="append", index=False)


def build_store_snapshot(detail_df):
...
def get_store_row(snapshot_df, code, course):
...
def ai_store_direction(hist_df):
    ...
    if last["북채"] == 0 or last["통날개"] == 0 or last["신선순살"] == 0:
...
def build_analysis_summary(curr_detail, prev1_detail=None, prev2_detail=None, prev3_detail=None):
    ...
    for (code, name, course), d in curr_detail.groupby(["매장코드", "매장명", "코스"]):
        ...
        def comp(prev_val, curr_val, item_name):
...
def get_assignment_df():
    return q("""
        ...
        ORDER BY bm_name, store_name
    """)


def assign_store(store_code, store_name, bm_name):
    ...
    row.to_sql("store_assignments", get_engine(), if_exists="append", index=False)


def team_ai_comment(team_df):
    total = len(team_df)
    urgent = int((team_df["소비기한리스크"] == "즉시확인").sum())
    caution = int((team_df["소비기한리스크"] == "주의").sum())

    if total == 0:
        return "분석 데이터가 없습니다."
    if urgent >= max(3, total * 0.2):
        return "운영2팀 전체 기준 즉시확인 매장 비중이 높습니다. 저발주·미발주 매장을 우선 확인하는 방향이 적절합니다."
    if caution >= max(5, total * 0.3):
        return "운영2팀 전체 기준 주의 매장이 다수 확인됩니다. 급감 패턴 중심으로 선별 관리가 필요합니다."
    return "운영2팀 전체 기준 발주 패턴은 비교적 안정적입니다. 이상 변동 매장만 선별 점검하는 방향이 적절합니다."


def current_user():
    default_user = st.session_state.get("user_name", st.query_params.get("user", ""))
...
def to_excel_bytes(summary_df, curr_detail, prev1_detail=None, prev2_detail=None, prev3_detail=None):
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if prev1_detail is not None and not prev1_detail.empty:
            prev1_detail.to_excel(writer, index=False, sheet_name="품목근거_전발주1")
        if prev2_detail is not None and not prev2_detail.empty:
            prev2_detail.to_excel(writer, index=False, sheet_name="품목근거_전발주2")
        if prev3_detail is not None and not prev3_detail.empty:
            prev3_detail.to_excel(writer, index=False, sheet_name="품목근거_전발주3")
    output.seek(0)
    return output.getvalue()


init_db()
st.title("BBQ 발주 분석 V4")
st.caption("웹공용 사용 기준 / 운영2팀 총합본 + 개인 활용 페이지 분리 / 저발주 중심 AI 판단 보조")

user_name = current_user()
menu = st.sidebar.radio(
    "메뉴",
    ["운영2팀 총합본", "오늘 발주 점검", "소비기한 리스크", "매장별 이력조회", "담당 매장 관리", "DB 관리"]
)
...
if menu == "오늘 발주 점검":
    ...
    if uploaded is not None:
        ...
        if st.button("분석 실행", type="primary", use_container_width=True):
            ...
            prev_dates = [d for d in dates if d < source_date][:3]

            prev1_detail = load_detail_by_date(prev_dates[0]) if len(prev_dates) > 0 else None
            prev2_detail = load_detail_by_date(prev_dates[1]) if len(prev_dates) > 1 else None
            prev3_detail = load_detail_by_date(prev_dates[2]) if len(prev_dates) > 2 else None
            ...
            if user_name:
                personal = summary_df[summary_df["BM"] == user_name].copy()
                if not personal.empty:
                    st.info(f"{user_name} 기준 매장만 표시합니다.")
                    display_df = personal
                else:
                    st.warning("현재 이름으로 배정된 매장이 없습니다. 담당 매장 관리에서 배정하세요.")

            st.dataframe(display_df, use_container_width=True, height=650)

            if save_flag:
                save_order_history(source_date, uploaded.name, curr_detail)
                st.success(f"공용 DB 저장 완료: {source_date}")

            excel_bytes = to_excel_bytes(summary_df, curr_detail, prev1_detail, prev2_detail, prev3_detail)
            st.download_button(
                "엑셀 다운로드",
                data=excel_bytes,
                file_name=f"발주점검_{source_date.replace('-', '')}_V4.xlsx",
    ...
    if not dates:
        ...
        selected_date = st.selectbox("발주일 선택", dates)
        curr_detail = load_detail_by_date(selected_date)
        ...
        st.dataframe(
            display_df[["매장명","코스","BM","신선육","전발주_신선육","최근3회평균_신선육","평균대비감소_신선육","미발주항목","감소항목","소비기한리스크","AI코멘트"]],
            use_container_width=True,
            height=700
        )
    ...
    if not dates:
        ...
        stores = q("SELECT DISTINCT store_name AS 매장명 FROM order_lines ORDER BY store_name")["매장명"].tolist()
        selected_store = st.selectbox("매장 선택", stores)
        ...
        if selected_store:
            hist = q("""
                ...
                FROM order_lines
                WHERE store_name = :s
                GROUP BY source_date, store_name
                ORDER BY source_date
            """, {"s": selected_store})

            st.dataframe(hist, use_container_width=True, height=550)
            st.info(ai_store_direction(hist))
    ...
    if not dates:
        ...
        bm_options = ["전체"] + bm_summary["BM"].tolist()
        selected_bm = st.selectbox("BM 선택", bm_options)
        view_df = summary_df.copy() if selected_bm == "전체" else summary_df[summary_df["BM"] == selected_bm].copy()

        st.markdown("#### 우선 관리 매장")
        urgent_df = view_df[view_df["소비기한리스크"].isin(["즉시확인", "주의"])].copy()
        st.dataframe(
            urgent_df[["BM","매장명","코스","미발주항목","감소항목","오일판정","소비기한리스크","총평"]].head(30),
            use_container_width=True,
            height=500
        )
    ...
    if not user_name:
        ...
        if st.button("내 담당 매장 저장", type="primary"):
            ...
            for code in selected_store_codes:
                ...
                assign_store(code, store_name, user_name)
    ...
    if dates:
        st.write(dates)
        delete_date = st.selectbox("삭제할 발주일 선택", dates)
        if st.button("선택 발주일 삭제"):
            exec_sql("DELETE FROM order_lines WHERE source_date = :d", {"d": delete_date})
            st.success(f"{delete_date} 삭제 완료")
            st.rerun()
