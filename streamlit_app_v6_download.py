...
def make_upload_key(source_date: str, uploader_name: str) -> str:
    clean = (uploader_name or "미지정").strip()
    return f"{source_date}_{clean}"
...
def get_store_row(snapshot_df: pd.DataFrame, code: int, course: str):
    if snapshot_df is None or snapshot_df.empty:
        return None
    x = snapshot_df[
        (snapshot_df["매장코드"] == int(code)) &
        (snapshot_df["코스"] == str(course))
    ]
...
if menu == "오늘 발주 점검":
    ...
    st.write("파일 생성본입니다. 나머지 로직은 위 구조와 동일하게 이어집니다.")
